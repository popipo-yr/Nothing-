...
- (void)configAboutSpecailDottedLineWithWidth:(CGFloat)width color:(UIColor *)color startPoint:(PYLinePoint)startPoint endPoint:(PYLinePoint)endPoint dottleInterval:(CGFloat)dottleInterval realInterval:(CGFloat)realInterval blankPercentage:(CGFloat)blankPercentage
{
    _isSpecail = true;
    ...
    _blankPercentage        = blankPercentage;
    
    _isDiagonalLine    =  (abs((int)(_startPointSpecial - _endPointSpecial)) == 1);
}


- (void)configAboutSpecailFullLineWithWidth:(CGFloat)width color:(UIColor *)color startPoint:(PYLinePoint)startPoint endPoint:(PYLinePoint)endPoint blankPercentage:(CGFloat)blankPercentage
{
    [self configAboutSpecailDottedLineWithWidth:width color:color startPoint:startPoint endPoint:endPoint dottleInterval:0 realInterval:10 blankPercentage:blankPercentage];
}
...
- (void)coverSpecailPointToNormalInBounds:(CGRect)bounds
{
    ...
    CGFloat percentageCo = _blankPercentage >= 0 ? _blankPercentage : (1 + _blankPercentage);

    if (_blankPercentage > 0) {  //空白在开始点
        if (_isDiagonalLine || _startPoint.y == _endPoint.y) {   //对角线或者水平 需要修改start的x
            _startPoint.x = _startPoint.x != 0 ? _startPoint.x * percentageCo : _endPoint.x * percentageCo;
        }

        if (_isDiagonalLine || _startPoint.x == _endPoint.x) {   //对角线或者垂直 需要修改start的y
            _startPoint.y = _startPoint.y != 0 ? _startPoint.y * percentageCo : _endPoint.y * percentageCo;
        }
    } else { //空白在结束点
        if (_isDiagonalLine || _startPoint.y == _endPoint.y) {   //对角线或者水平 需要修改end的x
            _endPoint.x = _endPoint.x != 0 ? _endPoint.x * percentageCo : _startPoint.x * percentageCo;
        }

        if (_isDiagonalLine || _startPoint.y == _endPoint.y) {   //对角线或者垂直 需要修改end的y
            _endPoint.y = _endPoint.y != 0 ? _endPoint.y * percentageCo : _startPoint.y * percentageCo;
        }
    }
}
